...
export default function DevLocationPage(){
  const [pos, setPos] = useState<{lat:number;lng:number}|null>({ lat:-27.996, lng:29.208 });
  const [addr, setAddr] = useState("");
  const [pc, setPc] = useState<{province?:string;city?:string}>({});
  const [suburb, setSuburb] = useState("");
  const [privacy, setPrivacy] = useState<Privacy>("CITY_ONLY");

  // recentre when city changes
  useEffect(()=>{
    (async ()=>{
      if(!pc.city) return;
      const q = [pc.city, pc.province, "South Africa"].filter(Boolean).join(", ");
      const r = await fetch(`/api/geocode?q=${encodeURIComponent(q)}${pc.province?`&province=${encodeURIComponent(pc.province)}`:""}`);
    ...
    <main style={{ maxWidth: "768px", margin:"0 auto", padding:"24px" }}>
      ...
      <Map
        ...
        onPick={async (p)=>{
          ...
          if(data?.province) setPc(prev=>({ ...prev, province: data.province }));
          if(data?.suburb) setSuburb(String(data.suburb));
        }}
      />
